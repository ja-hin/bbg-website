...
import { displaySerialNumber, isAutoGeneratedSerial } from "@/lib/utils";
...
export default function ClaimBBG() {
  ...
  useEffect(() => {
    if (claimDetails) {
      ...
      // Only set the serial number if it's not auto-generated
      if (!isAutoGeneratedSerial(claimDetails.customer.serialNumber)) {
                ...
                {claimDetails && (
                  <>
                    ...
                    <FormField
                      ...
                      name="serialNumber"
                      render={({ field }) => {
                        const isAutoGenerated = claimDetails?.customer?.serialNumber && isAutoGeneratedSerial(claimDetails.customer.serialNumber);
                        return (
                          <FormItem>
                            <FormLabel>Serial Number / IMEI *</FormLabel>
                            <FormControl>
                              <Input 
                                placeholder={isAutoGenerated 
                                  ? "Please complete device registration to continue" 
                                  : "Serial number will be auto-filled from registration"
                                }
                                readOnly
                                className="bg-gray-50 cursor-not-allowed"
                                {...field} 
                                value={isAutoGenerated ? "" : field.value}
                              />
                            </FormControl>
                            <FormMessage />
                            <p className="text-sm text-gray-500 mt-1">
                              {isAutoGenerated 
                                ? "You need to register your device with actual IMEI/Serial number before filing a claim"
                            ...
                            {isAutoGenerated && (
                              <div className="mt-2 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                                <p className="text-sm text-orange-800">
                                  <strong>Action Required:</strong> Please visit the device registration page to provide your actual device IMEI/Serial number before proceeding with your claim.
                                </p>
                              </div>
                            )}
                          </FormItem>
