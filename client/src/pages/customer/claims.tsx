  ...
  useEffect(() => {
    const checkAllEligibility = async () => {
      const results = await Promise.all(orders.map(async (order) => {
        // First do basic frontend checks
        if (!order.isVerified || order.claimStatus || !order.invoiceFile || order.invoiceFile === 'N/A' || isAutoGeneratedSerial(order.serialNumber)) {
          return null;
        }
        
        // Then do full backend eligibility check
        try {
      ...
      setEligibleOrders(results.filter(Boolean));
    };
