...
export async function registerRoutes(app: Express) {
  ...
  // Admin endpoint - Get customer registrations with filtering and payout calculation
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        // Get customers with basic filters
        const customers = await storage.getCustomerRegistrations({
        ...
        // Calculate estimated payout for each customer
        const customersWithPayout = await Promise.all(
          customers.map(async (customer) => {
            ...
            const deviceAgeMonths =
              (currentDate.getFullYear() - purchaseDate.getFullYear()) * 12 +
              (currentDate.getMonth() - purchaseDate.getMonth());

            // Get claim value slabs to calculate estimated payout
            let claimPercentage = 0;
            let estimatedPayout = 0;

            try {
              // Use preserved slab data from registration if available
              let slabData;
              if (customer.registrationSlabData) {
                slabData = JSON.parse(customer.registrationSlabData);
              } else {
                // Fallback to current active slabs
                const activeSlabs = await storage.getActiveClaimValueSlabs();
                slabData = activeSlabs.find(
                  (slab) =>
              ...
              if (slabData) {
                claimPercentage = slabData.claimPercentage || 0;
                estimatedPayout =
                  (customer.devicePrice * claimPercentage) / 100;
              }
            } catch (error) {
              console.error("Error calculating payout for customer:", error);
            }
